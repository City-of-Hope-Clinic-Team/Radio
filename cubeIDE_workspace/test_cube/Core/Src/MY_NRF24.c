...
{
	if(state) HAL_GPIO_WritePin(GPIOB, CSNpin_Pin, GPIO_PIN_SET);
	else HAL_GPIO_WritePin(GPIOB, CSNpin_Pin,  GPIO_PIN_RESET);
}
//2. Chip Enable
...
uint8_t NRF24_read_register(uint8_t reg)
{
	...
	//Receive data
	HAL_SPI_Receive(&hspi1, &spiBuf[1], 1, 100);
	retData = spiBuf[1];
	//Bring CSN high
	NRF24_csn(1);
	return retData;
...
void NRF24_write_register(uint8_t reg, uint8_t value)
{
	...
	spiBuf[0] = reg|0x20;
	spiBuf[1] = value;
	HAL_SPI_Transmit(&hspi1, spiBuf, 2, 100);
	//Bring CSN high
	NRF24_csn(1);
}
